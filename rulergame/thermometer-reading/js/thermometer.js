...
var stgTimer = admin_settings['stgTimer']?admin_settings['stgTimer']:sessionStorage.getItem("stgTimer");
var stgSounds = admin_settings['stgSounds']?admin_settings['stgSounds']:sessionStorage.getItem("stgSounds");
var stgUnit = admin_settings['stgUnit']?admin_settings['stgUnit']:sessionStorage.getItem("stgUnit");
var stgMode = admin_settings['stgMode']?admin_settings['stgMode']:sessionStorage.getItem("stgMode");
...
if (!stgSounds){
    stgSounds = "On";
    sessionStorage.setItem('stgSounds', stgSounds);
}

if(!stgUnit)
{
    stgUnit = "Fahrenheit";
    sessionStorage.setItem('stgUnit', stgUnit);
}
...
{
    stgMode = "Game";
    sessionStorage.setItem('stgMode', stgMode);
}
...
function checkGuess(guess) {
    ...
    {
        console.log(guess.toFixed(0) + "," + randomQuestion.toFixed(0));

        if (guess.toFixed(0) == randomQuestion.toFixed(0)) {
            showCheer();
...
function drawCorrection(correct, incorrect){
    ...
    if(stgMode == "Game" || stgMode == "Train"){
        ...
        $("#txtQuestion").html("<span class='questionWrongInstruction'>You selected</span><br><span class='questionWrongMeasurement'>" + incorrect + " " + stgSubScaleForML + "</span>").fadeIn();
    }
    else{
    ...
    if(0){
        ...
        let shift = parseInt(getMsrReading(correct), 10) * getMsdPixels();
...
function showNewQuestion() {
    ...
    if (stgUnit == "Fahrenheit")
    {
        while (randomQuestion == previousQuestion) 
            randomQuestion = randomInt(-15, 60) * 2;
    }
    else if (stgUnit	== "Celsius")
    {
        while (randomQuestion == previousQuestion) 
            randomQuestion = randomInt(-35, 50) * 1;
    }
	...
	if (stgMode == "Game" || stgMode == "Training") {
    	///////// modified by RUH on 1st ,Sep ////////////////////////////////////////////////////////////
    	html = "<span class='questionInstruction'>Control Thermometer to select</span><br>";
        //////////////////////////////////////////////////////////////////////////////////////////////////
    
        $("#txtQuestion").html(html + "<span class='questionMeasurement'>" + randomQuestion.toFixed(0) + " " + stgSubScaleForML + " </span>").fadeIn();
	}
    else{
            html = "<span class='questionInstruction'>Enter the temperature.</span>";
