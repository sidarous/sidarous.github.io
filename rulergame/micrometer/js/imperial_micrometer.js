...
var session_prefix ="micrometer_"
...
var sleeve_main_scale_value = 0.5;
...
function DrawMicrometer(sleeve_top_left_x, sleeve_top_left_y, sleeve_bottom_left_x, sleeve_bottom_left_y, sleeve_top_right_x, sleeve_top_right_y, sleeve_bottom_right_x, sleeve_bottom_right_y, thimble_mid_top_x, thimble_mid_top_y, thimble_mid_bottom_x, thimble_mid_bottom_y, thimble_right_top_x, thimble_right_top_y, thimble_right_bottom_x, thimble_right_bottom_y, thimble_start_y)
{
    ...
    for (let k = 0; k < 41; k++)
    {
        if ( sleeve_top_left_x + k*10 < sleeve_top_right_x )
        {
            if ( k % 4 == 0)
            {
                ctx.moveTo(sleeve_top_left_x + k*10, sleeve_main_scale_start_y);
                ctx.lineTo(sleeve_top_left_x + k*10, sleeve_main_scale_start_y + 50);
                ctx.font = sleeve_main_Font;
                ctx.fillText(k / 4, sleeve_top_left_x + k*10 -10, sleeve_main_scale_start_y + 80);
            }
            else if( k % 2 == 0)
            {
                ctx.moveTo(sleeve_top_left_x + k*10, sleeve_main_scale_start_y);
                ctx.lineTo(sleeve_top_left_x + k*10, sleeve_main_scale_start_y + 30);
            }
            else
            {
                ctx.moveTo(sleeve_top_left_x + k*10, sleeve_main_scale_start_y);
                ctx.lineTo(sleeve_top_left_x + k*10, sleeve_main_scale_start_y + 15);
            }
        }
        else
        {
    ...
    for (let j = 0; j < 10; j++)
    {
        ...
        for ( let k = 0; k < 41; k++)
        {
            ...
            {
                sleeve_small_scale_value = (9-j)/10000;
            }
        }
    }
    sleeve_main_scale_value = Math.round(1000*sleeve_main_index*0.025)/1000;
    ...
    {
        micrometer_value = Math.round(1000*(sleeve_main_scale_value + thimble_scale_value))/1000;
    }
    //micrometer_value = Math.round(10000*(sleeve_main_scale_value + thimble_scale_value + sleeve_small_scale_value))/10000;
...
function drawType(){
    ...
    if (stgMode == "Type") {
        ...
        
        {
            ...
            outString(ctx, show_num_start_x + 40 * 3 + 34, show_num_start_y + 20, ".", 0, 0);
...
function GetMicrometerCoordinates(micrometer_value) {
    const sleeve_main_scale_value = Math.floor(micrometer_value / 0.025) * 0.025;
    
    const sleeve_main_index = sleeve_main_scale_value / 0.025;
    const sleeve_top_right_x = sleeve_top_left_x + sleeve_main_index * 10 + 2;
    
    const remainder_after_main = micrometer_value - sleeve_main_scale_value;

    // Step 2: Calculate thimble_scale_value (step 0.001)
    const thimble_scale_value = Math.floor(remainder_after_main / 0.001) * 0.001;
    ...
    if (stgVernierScale === "On") {
        sleeve_small_scale_value = Math.round(remainder_after_thimble / 0.0001) * 0.0001;
    }

    // Calculate total thimble displacement (0.001 + 0.0001)

    thimble_start_y = sleeve_bottom_right_y - (35-thimble_scale_value/0.001)*10;
    ...
    var sleeve_small_interval = sleeve_small_scale_value / 0.0001;
...
function showNewQuestion() {
    ...
    while (randomQuestion === previousQuestion) {
        if(stgVernierScale=="On")
        {
            randomQuestion = randomInt(1, 9999) / 10000;
        }
        else
        {
            randomQuestion = randomInt(1, 999) / 1000;
        }
...
{
    console.log("DrawSleeveMainChange",sleeve_bottom_left_y)
    var sleeve_main_val = $("#sleevemainControl").val();
    // alert(sleeve_main_val);
    sleeve_top_right_x = sleeve_top_left_x + (sleeve_main_val/0.025+0.2)*10;
    sleeve_bottom_right_x = sleeve_top_right_x;
    thimble_mid_top_x = sleeve_top_right_x + 100;
    thimble_mid_bottom_x = thimble_mid_top_x;
    thimble_right_top_x = thimble_mid_top_x + 100;
...
function DrawSleeveSmallChange()
{
    ...
    // sleeve_top_right_x = sleeve_top_right_x + move_x;
    var sleeve_small_interval = sleeve_small_val / 0.0001;
...
$("#btnsleevedecrement").click(function(){
    //inputType = "Mouse";
    var sleeve_main_val = $("#sleevemainControl").val();
    sleeve_main_val = Math.round(sleeve_main_val * 1000)
    sleeve_main_val -= 25;
    ...
    $("#sleevemainControl").val(sleeve_main_val/ 1000);
...
$("#thimbledecrement").click(function(){
    //inputType = "Mouse";
    var thimble_val = $("#thimbleControl").val();
    thimble_val = Math.round(thimble_val * 1000);
    thimble_val -= 1;
    if (thimble_val <= 0)
    {
        thimble_val = 0;
    ...
    $("#thimbleControl").val(thimble_val/1000);
...
$("#vernierdecrement").click(function(){
    ...
    vernier_val = Math.round(vernier_val * 10000);
    vernier_val -= 1;
    if (vernier_val <= 0)
    {
        vernier_val = 0;
    ...
    $("#sleevesmallControl").val(vernier_val/10000);
