...
(function () {
  ...
  function parseStudentsCsv(csvText) {
    ...
    if (rows.length < 2) throw new Error(`${STUDENT_CSV_URL} contains no student records`);
    ...
    if (!students.length) throw new Error(`${STUDENT_CSV_URL} contains no student records`);
    return students;
  }
  ...
  async function loadStudentsFromCsv() {
    ...
    if (typeof window.DEFAULT_ROSTER_CSV === "string" && window.DEFAULT_ROSTER_CSV.trim()) {
      return parseStudentsCsv(window.DEFAULT_ROSTER_CSV);
    }

    if (window.location.protocol === "file:") {
      const error = new Error("The bundled default roster is unavailable; choose a roster CSV to continue");
      error.code = "ROSTER_IMPORT_REQUIRED";
      throw error;
    }

    const response = await fetch(STUDENT_CSV_URL, { cache: "no-store" });
    if (!response.ok) throw new Error(`Could not load ${STUDENT_CSV_URL} (${response.status})`);
  ...
  function setupRosterImportControls() {
    ...
    const defaultRosterButton = document.getElementById("btn-use-default-roster");
    ...
    if (defaultRosterButton) {
      defaultRosterButton.addEventListener("click", () => {
        localStorage.removeItem(STORAGE_KEYS.ROSTER_CSV);
        localStorage.removeItem(STORAGE_KEYS.ROSTER_META);
        const status = document.getElementById("settings-roster-status");
        if (status) {
          status.className = "roster-import-status success";
          status.textContent = "Default roster restored. Reloading…";
        }
        window.setTimeout(() => window.location.reload(), 450);
      });
    }
  ...
  function updateRosterSummary() {
    ...
    if (metadata) {
      ...
      summary.textContent = `${state.students.length.toLocaleString()} students from the bundled default roster`;
    }
  }
